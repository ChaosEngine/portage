...
src_configure() {
	...
	if use sse; then
		myesconsargs=(${myesconsargs} -j2 --lua-dir=/usr/include --sse --sse2 --sse3 --lin --release CC="$(tc-getCC)")
	else
		myesconsargs=(${myesconargs} -j2 --lua-dir=/usr/include --lin --release CC="$(tc-getCC)")
	fi
}
